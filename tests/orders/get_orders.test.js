...
async function generateAccessToken() {
  ...
  if (authResponse.status == 200) {
    ...
    console.debug(
      `New ACCESS_TOKEN "${ACCESS_TOKEN}" is successfully generated and preserved locally.`
    )
  } else if (authResponse.status === 409) {
    console.debug(
      `Current ACCESS_TOKEN "${ACCESS_TOKEN}" is still valid. API client is already registered. `
    )
  } else {
    throw Error(
